...
const KitchenHome = () => {
  ...
  const handleAccept = (e) => {
    ...
    axios
      .post(`http://localhost:4000/postkitchendata/`, payload, {
        header: {
          "Content-type":
      ...
      .then((response) => {
        ...
        if (response.status === 200) {
          ...
          let objIndex = kitchenData.findIndex(
          ...
          setKitchenData((prev) =>
            prev.map((obj) => array.find((o) => o._id === obj._id) || obj)
          );
        }
      })
  ...
  console.log(
    "outside",
    kitchenData.map((item) => item.orderStatus)
  );
  return (
    <div className="inner-container">
      {loading ? (
        <div style={{ marginTop: "290px" }}>
          <ClipLoader color="#1f305e" loading={true} css={""} size={100} />
        </div>
      ) : (
                ...
                <tr
                  ...
                  style={
                    eachOrder.orderStatus === "accept" ||
                      eachOrder.orderStatus === "complete"
                      ? { background: "#d9ffb3" }
                    ...
                    {eachOrder.orderStatus === "accept" ? (
                      <div>
                        ...
                        <section className="timer">
                          <Countdown
                            date={
                              Date.now() + JSON.parse(eachOrder.required_time)
                            }
                          />
                        </section>
                      ...
                      <h5 className="blink">Order Completed</h5>
                    ) : (
                      <>
                        <div>
                          <Form>
                            <Form.Group controlId="exampleForm.ControlSelect1">
                              ...
                              >
                                ...
                                <option value="120000">20 min</option>
                  ...
                  <td>
                    <ol>
                      {eachOrder.items.map((val, index) => (
                        <li key={index}>
                          <span className="order-name">{val.itemName}</span>----
                          <span className="order-name">{val.quantity}</span>
                        </li>
                      ))}
                    </ol>
                  </td>
